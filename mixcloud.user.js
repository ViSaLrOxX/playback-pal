// ==UserScript==
// @name         playback-pal · Mixcloud
// @namespace    https://github.com/ViSaLrOxX/playback-pal
// @version      1.0.0
// @description  Playback speed controls for Mixcloud
// @author       ViSaLrOxX
...
(function () {
  ...
  function getAudio() {
    return document.querySelector('audio[data-testid="audioTag"], audio');
  }

  function forceRate(el, val) {
    if (!el) return;
    lockActive = true;
    try {
      el.playbackRate        = val;
  ...
  function attachTo(el) {
    if (!el || el.__ppAttached) return;
    el.__ppAttached = true;
    forceRate(el, speed);
    el.addEventListener('ratechange',     () => { if (!lockActive && el.playbackRate !== speed) forceRate(el, speed); });
    el.addEventListener('play',           () => forceRate(el, speed));
    el.addEventListener('canplay',        () => forceRate(el, speed));
    el.addEventListener('loadedmetadata', () => forceRate(el, speed));
    el.addEventListener('seeking',        () => forceRate(el, speed));
  }
  ...
  setInterval(() => {
    ...
    if (a) {
      attachTo(a);
      if (!lockActive && a.playbackRate !== speed) forceRate(a, speed);
    }
  }, 500);
  ...
  setInterval(() => {
    ...
    if (title && title !== lastTrack) {
      lastTrack = title;
      [300, 800, 1500].forEach(t => setTimeout(() => forceRate(getAudio(), speed), t));
    }
  }, 500);
  ...
  GM_addStyle(`
    ...
    #pp-cur {
      ...
      border: 1px solid rgba(80,0,255,0.25);
      border-radius: 5px;
      padding: 1px 8px 2px;
    }
    #pp-hide {
    ...
    .ppb.active {
      background: rgba(80,0,255,0.14);
      border-color: rgba(80,0,255,0.38);
      color: #5000ff;
    }
    #pp-custom {
      display: flex;
    ...
    #pp-custom-input:focus { border-color: rgba(80,0,255,0.4); }
    ...
    #pp-custom-btn:hover { background: rgba(80,0,255,0.22); }
    #pp-sub {
      font-size: 9px;
